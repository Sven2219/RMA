...
import { addNew, updateByID } from '../helpers/sqlHelper';
...
const AddInspiringPersonContainer = (props: Props) => {
    ...
    React.useEffect(() => {
        if (props.activeIndex !== undefined) {
            ...
            if (state.inspiringPersons[props.activeIndex].uri) {
                //@ts-ignore
                setImageURI(state.inspiringPersons[props.activeIndex].uri)
            }
            setQuotes(state.inspiringPersons[props.activeIndex].quotes);
        }
    }, [])
    ...
    const handleButtonPress = React.useCallback((data: Person) => {
        ...
        if (state.inspiringPersons.length > 0) {
            id = state.inspiringPersons[state.inspiringPersons.length - 1].id + 1;
        }
        const person: Person = {
            id,
        ...
        if (props.activeIndex !== undefined && props.setActiveIndex) {
            ...
            const formatedQuotes = quotes.join(";");
            updateByID(db, state.inspiringPersons[props.activeIndex].id, person, formatedQuotes);
            dispatch({ type: ActionTypes.SET_INSPIRING_PERSONS, payload: updatedPersons });
            props.setActiveIndex(CLOSED_MODAL)
        } else {
            const formatedQuotes = quotes.join(";");
            addNew(db, imageURI, data, formatedQuotes);
